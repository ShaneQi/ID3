...
public class Entropy {
	...
	Entropy(List<List<String>> e, List<String> a,String t){
		element =e;
		att = a;
		tarAtt = t;
	}
	...
	public int[] entropy(){
		...
		for(int j = 0; j < attlen-1;j++){
			...
			//impurity
			//entrol = att0left[j]*att0right[j]/Math.sqrt(att0left[j]+att0right[j]);
			//entropy
			if (xl == 0 || yl == 0) entrol = 0;
			else entrol = -(xl*(Math.log(xl)/Math.log(2)) + yl*(Math.log(yl)/Math.log(2)));
			}
			...
			//impurity
			//entror = att1left[j]*att1right[j]/Math.sqrt(att1left[j]+att1right[j]);
			//entropy
			if (xr == 0 || yr == 0)  entror = 0;
			else entror = -(xr*(Math.log(xr)/Math.log(2)) + yr*(Math.log(yr)/Math.log(2)));
			}
